...
r.raise_for_status()
with open('supported_extensions.js', 'w') as se_f:
    se_f.write('const SUPPORTED_EXTENSIONS = {};'.format(r.text.rstrip('\n')))

files = [f for f in os.listdir('.') if os.path.isfile(f) and not f.endswith('.py')]
for f in ['.gitignore', '.travis.yml', 'LICENSE', 'README.md', 'package.json', 'package-lock.json', 'requirements.txt', 'setup.cfg']:
    files.remove(f)
